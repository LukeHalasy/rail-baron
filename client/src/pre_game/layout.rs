...
pub fn Layout(
    ...
    // pull in the error
    let error = use_context::<ReadSignal<Option<Error>>>().expect("Expected an error");
        ...
        <div class="fixed top-0 left-0 flex justify-center w-full h-full bg-black bg-opacity-40">
            <div class="flex flex-col items-center mt-20">
                ...
                <div class="flex flex-col items-center justify-center p-6 space-y-3 bg-gray-400 w-80">
                    {children()}
                    <p class="text-white bg-slate-800">{move || error.get().map(|error| error.to_string()).unwrap_or_default()}</p>
                </div>
            </div>
        </div>
    }
